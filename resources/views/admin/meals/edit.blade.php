            ...
            <form class="bg-white" method="POST" action="{{ route('admin.meals.update', $meal->id) }}"
                ...
                <div class="mb-6">
                    <label for="prix" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Price:</label>
                    <input type="number" name="prix" id="prix" value="{{ $meal->prix }}"
                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light  @error('prix') border-red-500 @enderror ">
                    @error('prix')
                        <div class="text-sm text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="image">Upload
                        IMG</label>
                    <div>
                        <img class="w-32 h-32" src="{{ asset($meal->image) }}" alt="">
                    </div>
                    <input
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" rows="3"
                        ...
                        placeholder="Write your thoughts here..."> {{ $meal->description }}</textarea>
                    @error('description')
                        <div class="text-sm text-red-500">{{ $message }}</div>
                    @enderror
